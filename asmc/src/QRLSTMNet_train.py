...
from keras.layers import Dense, Dropout, Activation, Flatten, Conv2D, MaxPooling2D, BatchNormalization, Lambda, Input, \
    Concatenate, LSTM, LeakyReLU, Reshape, Add
...
# ----------------------- QRLSTMNet ---------------------------
####################### EXPLANATION #########################
# We take the LER prediction from edgenet and the error prediction from
# our NCPLSTMNet and try to fit this to Quantile estimates using
# a neural net.
...
def Model_Builder(miscoverage: float):
    ...
    llayer1 = Dense(4)(input1)
    llayer1 = LeakyReLU()(llayer1)

    rlayer1 = Dense(4)(input1)
...
X_train_path = path_asmc + "dataset/" + "X_train_QR_train2.npy"
...
history = left_model.fit(Xtrain[:, :2], [ytrain[:, 0], ytrain[:, 0]],
                         batch_size=18,
                         epochs=4,
                         shuffle=True,
                         callbacks=callbacks)

left_model.save(path_asmc + 'models/' + f'QRLSTMNet_ledge_{coverage_rate_string}.h5')

print("**************************Now training rigt edge************************************")
right_model = Model_Builder(miscoverage_rate)

history = right_model.fit(Xtrain[:, 2:], [ytrain[:, 1], ytrain[:, 1]],
                          batch_size=18,
                          epochs=4,
                          shuffle=True,
                          callbacks=callbacks)

right_model.save(path_asmc + 'models/' + f'QRLSTMNet_redge_{coverage_rate_string}.h5')
