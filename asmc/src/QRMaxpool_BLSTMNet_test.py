...
model_left = load_model(path_asmc + 'models/' + 'QRMaxpool_BLSTMNet_ledge_90.h5', compile=False)
model_left.compile('adam', loss=[tfa.losses.PinballLoss(tau=0.05), tfa.losses.PinballLoss(tau=0.95)])
model_right = load_model(path_asmc + 'models/' + 'QRMaxpool_BLSTMNet_redge_90.h5', compile=False)
model_right.compile('adam', loss=[tfa.losses.PinballLoss(tau=0.05), tfa.losses.PinballLoss(tau=0.95)])


# (edgenet_ler, normcp_approach2, normcp_natlog5)
...
df_test.to_pickle(path_asmc + 'models/' + "Test_df_QR_approach6_90")

print("Calibration count: ", cal_count)

print(df.describe())

df.to_pickle(path_asmc + 'models/' + "Calibration_df_QR_approach6_90")
...
df1 = pd.read_pickle(path_asmc + 'models/' + "Calibration_df_QR_approach6_90")
df_test1 = pd.read_pickle(path_asmc + 'models/' + "Test_df_QR_approach6_90")
