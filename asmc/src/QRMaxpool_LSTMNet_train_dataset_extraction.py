...
############################# description ##########################################
# In the previous dataset we had 2 features: (edgenet_ler, NCPLSTMNet estimate). In this
# dataset we have 3 features: (edgenet_ler, NCPLSTMNet estimate, NCPMaxpool estimate).
##################################################################################

path = '/scratch/user/nini16/SEM/'
path_asmc = '/scratch/user/nini16/ASMC_SEM/'

X_train_path = path_asmc + "dataset/" + "X_train_QRMaxpool_LSTMNet.npy"
y_train_path = path_asmc + "dataset/" + "y_train_QR.npy"
...
ncplstmnet_left = load_model(path_asmc + 'models/' +'NormCP_approach1_ledge.h5')
ncplstmnet_right = load_model(path_asmc + 'models/' +'NormCP_approach1_redge.h5')
ncpmaxpoolnet = load_model(path + 'models/' + 'NoiseCompression_final_model_5.h5')

# Extraction
num_training = 9920*9
X_train      = np.zeros((num_training, 6))
            ...
            for width in widths:
                for s in range(2):
                    for noise in noises:
                        ...
                        X_train[count,3] = ler_pred[1][0]
                        
                        X_train[count,1] = left[0]
                        X_train[count,4] = right[0]
                        
                        X_train[count,2] = natlog5[0]
                        X_train[count,5] = natlog5[1]
