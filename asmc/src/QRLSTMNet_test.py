...
model_left = load_model(path_asmc + 'models/' + 'QRLSTMNet_ledge_90.h5', compile=False)
model_left.compile('adam', loss=[tfa.losses.PinballLoss(tau=0.05), tfa.losses.PinballLoss(tau=0.95)])
model_right = load_model(path_asmc + 'models/' + 'QRLSTMNet_redge_90.h5', compile=False)
            ...
            for width in widths:
                for s in range(2):
                    ...
                    for noise in noises:
                        ...
                        input2[0,2] = ler_pred[1][0]
                        
                        input2[0,1] = left[0]
                        input2[0,3] = right[0]
                        
                        left = np.array( model_left.predict(input2[:,:2]) ).reshape((2,))
                        right = np.array( model_right.predict(input2[:,2:]) ).reshape((2,))
...
df_test.to_pickle(path_asmc + 'models/' + "Test_df_QR_approach2_90")

print("Calibration count: ", cal_count)

print(df.describe())

df.to_pickle(path_asmc + 'models/' + "Calibration_df_QR_approach2_90")
...
df1 = pd.read_pickle(path_asmc + 'models/' + "Calibration_df_QR_approach2_90")
df_test1 = pd.read_pickle(path_asmc + 'models/' + "Test_df_QR_approach2_90")
