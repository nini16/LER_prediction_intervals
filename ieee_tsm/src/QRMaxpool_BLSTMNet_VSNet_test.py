...
path_asmc = '/scratch/user/nini16/ASMC/'
...
ncpmaxpoolnet = load_model(path_asmc + 'models/' + 'NCPMaxpool.h5')
ncpblstmnet_left = load_model(path_asmc + 'models/' +'NCPBLSTMNet_ledge.h5')
ncpblstmnet_right = load_model(path_asmc + 'models/' +'NCPBLSTMNet_redge.h5')

model_left = load_model(path_tsm + 'models/' + 'QRMaxpool_BLSTMNet_VSNet_ledge_95.h5', compile=False)
model_left.compile('adam', loss=[tfa.losses.PinballLoss(tau=0.025), tfa.losses.PinballLoss(tau=0.975)])
model_right = load_model(path_tsm + 'models/' + 'QRMaxpool_BLSTMNet_VSNet_redge_95.h5', compile=False)
model_right.compile('adam', loss=[tfa.losses.PinballLoss(tau=0.025), tfa.losses.PinballLoss(tau=0.975)])

# (edgenet_ler_prediction, NCPBLSTMNet_prediction, NCPMaxPoolNet_prediction, noise_level_param)
            ...
            for width in widths:
                for s in range(2):
                    ...
                    for noise in noises:
                        ...
                        left2 = ncpblstmnet_left.predict(diff)[0]
                        right2 = ncpblstmnet_right.predict(diff)[0]
                        ...
                        noise_param = skimage.restoration.estimate_sigma(noisy_temp)
                        input2[0,6] = noise_param
                        input2[0,7] = noise_param
                        
                        left = np.array( model_left.predict(input2[:,[0,1,2,6]]) ).reshape((2,))
                        right = np.array( model_right.predict(input2[:,[3,4,5,7]]) ).reshape((2,))
...
df_test.to_pickle(path_tsm + 'models/' + "Test_df_QRMaxpool_BLSTMNet_VSNet_95")

print("Calibration count: ", cal_count)

print(df.describe())

df.to_pickle(path_tsm + 'models/' + "Calibration_df_QRMaxpool_BLSTMNet_VSNet_95")
...
df1 = pd.read_pickle(path_tsm + 'models/' + "Calibration_df_QRMaxpool_BLSTMNet_VSNet_95")
df_test1 = pd.read_pickle(path_tsm + 'models/' + "Test_df_QRMaxpool_BLSTMNet_VSNet_95")
